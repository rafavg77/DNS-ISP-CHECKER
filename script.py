...
def query_dns(IP,NAME,CITY):
	...
	p = subprocess.Popen("nslookup -query=A um.edu.mx %s" % IP ,stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
	p = subprocess.Popen("nslookup -query=NS um.edu.mx %s" % IP ,stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
